...
@section('input')
    @if($formGroup)<div @if ($groupClass){!! 'class="' . $groupClass . '"' !!}@endif>@endif
        {!! $before !!}
        @if($label)<label for="{{ $options['id'] }}" @if ($labelClass){!! 'class="' . $labelClass . '"' !!}@endif>{!! $label !!}</label>@endif
        @yield('input')
        @if($labelAfter)<label for="{{ $options['id'] }}" @if ($labelClass){!! 'class="' . $labelClass . '"' !!}@endif>{!! $labelAfter !!}</label>@endif
